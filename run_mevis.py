...
def test(args=None):
    ...
    # Auto-detect nested MeViS_release subfolder
    if os.path.isdir(os.path.join(root, 'MeViS_release')):
        root = os.path.join(root, 'MeViS_release')
    ...
    # Fallback: search common meta file locations
    if not os.path.exists(meta_file):
        candidates = [
            os.path.join(root, 'meta_expressions.json'),
            os.path.join(root, 'valid', 'meta_expressions.json'),
        ]
        for c in candidates:
            if os.path.exists(c):
                meta_file = c
    ...
    for idx_, video in enumerate(video_list):
        ...
        for e in range(num_expressions):
            ...
            ref_num = max(10, num_references * 2) if is_multi_ref else 10
            ...
            if tracker == 'sam2':
                ...
                best_i = int(best_ref_idx * (video_len - 1) / (10 - 1))
    ...
    zip_path = os.path.join(os.path.dirname(os.path.abspath(output_dir)), 'MeViS_results')
