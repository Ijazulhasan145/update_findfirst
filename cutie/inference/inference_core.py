...
class InferenceCore:
    ...
    def initialize_memory(self, selected_references):
        self.clear_memory()
        self.memory.num_references = len(selected_references)
        self.memory.reference_weights = [ref.weight for ref in selected_references]
        
        for ref in reversed(selected_references):
            image = ref.image
            mask = ref.mask
            objects = [1]
            ...
            last_mask = pred_prob_with_bg[1:].unsqueeze(0)
            if self.flip_aug:
                last_mask = torch.cat([last_mask, torch.flip(last_mask, dims=[-1])], dim=0)
                
            ms_feat, pix_feat = self.network.encode_image(image)
            key, shrinkage, selection = self.network.transform_key(pix_feat)
    ...
    def reinject_reference(self, image, mask, frame_idx, objects=[1]):
        self.curr_ti = frame_idx
        self.last_mem_ti = frame_idx
