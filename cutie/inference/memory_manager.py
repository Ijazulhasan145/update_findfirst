    ...
    def read(self, pix_feat: torch.Tensor, query_key: torch.Tensor, selection: torch.Tensor,
        ...
        for bucket_id, bucket in buckets.items():
            if self.num_references > 1:
                p_size = self.work_mem.perm_size(bucket_id)
                k_all = self.work_mem.k[bucket_id]
                s_all = self.work_mem.s[bucket_id]
                e_all = self.work_mem.e[bucket_id] if self.save_selection else None
                
                total_size = k_all.shape[-1]
                temp_size = total_size - p_size
                
                if temp_size > 0:
                    k_temp = k_all[:, :, p_size:]
                    s_temp = s_all[:, :, p_size:]
                    e_temp = e_all[:, :, p_size:] if e_all is not None else None
                else:
                    k_temp = None
                    s_temp = None
                    e_temp = None
                ...
                for objects in object_chunks:
                    ...
                    val_temp = this_msk_value[:, :, :, p_size:] if temp_size > 0 else None
                    ...
                    weighted_readout_memory = None
                    
                    for idx in range(self.num_references):
                        start_idx = idx * self.HW
                        end_idx = (idx + 1) * self.HW
                        
                        k_ref = k_all[:, :, start_idx:end_idx]
                        s_ref = s_all[:, :, start_idx:end_idx]
                        val_ref = this_msk_value[:, :, :, start_idx:end_idx]
                        
                        if e_all is not None:
                            e_ref = e_all[:, :, start_idx:end_idx]
                        else:
                            e_ref = None
                            
                        if temp_size > 0:
                            k_i = torch.cat([k_ref, k_temp], dim=-1)
                            s_i = torch.cat([s_ref, s_temp], dim=-1)
                            val_i = torch.cat([val_ref, val_temp], dim=-1)
                            if e_all is not None:
                                e_i = torch.cat([e_ref, e_temp], dim=-1)
                            else:
                                e_i = None
                        else:
                            k_i = k_ref
                        ...
                        similarity_i = get_similarity(k_i, s_i, query_key, e_i if e_all is not None else selection)
                        affinity_i = do_softmax(similarity_i, top_k=self.top_k, inplace=True)
                            
                        visual_readout_i = self._readout(affinity_i, val_i).view(bs, len(objects), self.CV, h, w)
                        pixel_readout_i = network.pixel_fusion(pix_feat, visual_readout_i, this_sensory, this_last_mask)
                        readout_memory_i, aux_features_i = network.readout_query(pixel_readout_i, this_obj_mem)
                        
                        weight_i = self.reference_weights[idx]
                        if weighted_readout_memory is None:
                            weighted_readout_memory = weight_i * readout_memory_i
                        else:
                            weighted_readout_memory += weight_i * readout_memory_i
                            
                    for i_obj, obj in enumerate(objects):
                        all_readout_mem[obj] = weighted_readout_memory[:, i_obj]
            else:
